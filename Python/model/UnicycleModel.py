...
class unicycle3(unicycle1):
    ...
    def forward_uncertain(self,x,u,w,idx=None):
        ...
        # f[:,1] = (v + 0.1*w1) * np.sin(x3)
        # f[:,2] = omega + 0.1*w2
        f[:,0] = v * np.cos(x3 + self.c1*w1)
        f[:,1] = v * np.sin(x3 + self.c1*w1)
        f[:,2] = omega + self.c2*w2
    ...
    def diff_F(self,x,u,w):
        ...
        fw = np.zeros((N,self.ix,2))
        fw[:,0,0] = -self.c1*uv*np.sin(x3 + self.c1*w1)
        fw[:,0,1] = 0.0
        # fw[:,1,0] = self.c1*np.sin(x3 + self.c1*w1)
        fw[:,1,0] = self.c1*uv*np.cos(x3 + self.c1*w1)
        fw[:,1,1] = 0.0
        fw[:,2,0] = 0.0
        fw[:,2,1] = self.c2

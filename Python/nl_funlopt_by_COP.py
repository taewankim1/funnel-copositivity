...
class nl_funlopt_by_COP :
    ...
    def cvx_initialize(self,lambda_w,Qini=None,Qf=None,
        ...
        beta_squared = []
        for i in range(N) :
            gamma_squared.append(cvx.Parameter(pos=True))
            beta_squared.append(cvx.Parameter(pos=True))
        ...
        def get_N1(i): #,gamma_inv,beta_inv) :
            ...
            N1 = cvx.hstack((lam[i,0]*np.eye(ix+iu+iw),np.zeros((ix+iu+iw,iq))))
            N2 = cvx.hstack((np.zeros((iq,ix+iu+iw)),lam[i,1]*np.eye(iq)))
            N = cvx.vstack((N1,N2))
            return N

        def get_N2(i,gamma_sq,beta_sq) :
            N1 = cvx.hstack((lam[i,0]*beta_sq*np.eye(ix),np.zeros((ix,iphi))))
            N2 = cvx.hstack((np.zeros((iphi,ix)),lam[i,1]*gamma_sq*np.eye(iphi)))
            N = cvx.vstack((N1,N2))
            return N

        def get_H(i,j,dQ,gamma_sq,beta_sq) :
            ...
            LMI11 = Wij + Wji - 2*dQ
            LMI21 = (get_N2(i,gamma_sq,beta_sq) + get_N2(j,gamma_sq,beta_sq)) @ E.T
            LMI31 = F[i].T + F[j].T
            LMI41 = Li + Lj
            LMI22 = - (get_N2(i,gamma_sq,beta_sq)+get_N2(j,gamma_sq,beta_sq))
            LMI32 = np.zeros((iw,ip))
            LMI42 = np.zeros((ir,ip))
            LMI33 = -2*lambda_w * np.eye(iw)
            LMI43 = 2*G
            LMI44 = - (get_N1(i) + get_N1(j))
        ...
        def copositive_to_LMI_1(constraints) :
            for i in range(N) :
                ...
                gamma_sq = gamma_squared[i]
                beta_sq = beta_squared[i]

                H_ii = get_H(i,i,dQ,gamma_sq,beta_sq)
                constraints.append(H_ii >> 0)

                H_ij_ji = get_H(i,j,dQ,gamma_sq,beta_sq)
                constraints.append(H_ij_ji >> 0)

                H_jj = get_H(j,j,dQ,gamma_sq,beta_sq)
        ...
        self.cvx_params['F'] = F
        self.cvx_params['G'] = G
        self.cvx_params['gamma_squared'] = gamma_squared
        self.cvx_params['beta_squared'] = beta_squared
    ...
    def cvxopt(self,gamma,beta,A,B,C,D,E,F,G) :
        ...
        for i in range(N+1) :
            ...
            if i < N :
                self.cvx_params['gamma_squared'][i].value = (gamma[i]**2)
                self.cvx_params['beta_squared'][i].value = (beta[i]**2)
        ...
        data, _, _ = self.prob.get_problem_data(cvx.CLARABEL)
        print("---")
        print(data["dims"])
        print("---")
